...
router.put("/:user_id/:id", authenticate, async (req, res) => {
  ...
  try {
    ...
    // remove old
    await db("form_sections").where("form_id", id).del();
...
router.delete("/:user_id/:id", authenticate, async (req, res) => {
  ...
  try {
    ...
    await db("form_sections").where("form_id", parseInt(id)).del();
...
router.get("/response/:user_id/:id", authenticate, async (req, res) => {
  ...
  try {
    ...
    const responses = await db("form_responses")
      .join(
        "form_sections",
        "form_responses.form_section_id",
        "=",
        "form_sections.id"
      )
      ...
      .select(
        "form_responses.id",
        "form_responses.content",
        "form_responses.created",
        "form_responses.form_section_id",
        "form_sections.label"
      );
    ...
    const formattedResponses = responses.map((response) => {
      const responseObj = {
        form_id: id,
        form_section_id: response.form_section_id,
...
router.post("/response/:user_id/:id", async (req, res) => {
  ...
  try {
    ...
    if (!form) {
      return res
        .status(404)
        .json({ message: "Form not found or not accessible." });
    }
